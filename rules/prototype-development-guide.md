...
向用户请求验收或反馈时优先使用 `serverUrl`；管理服务尚未就绪时才使用 `targetUrl`。`/prototypes/<原型目录>` 只作为脚本参数或运行时路径。

验收移动端或平板端原型时，在 `serverUrl` 上使用对应的设备参数：移动端 `device=393x852`，平板端 `device=820x1180`。
