...
export default function BatchDetail({ batch, onClose, onTerminate, onViewRecords }: BatchDetailProps) {
    ...
    const progress =
        status === '执行中' && batch.systemVerifiedCount !== null && batch.executedCount !== null
            ? Math.min((batch.executedCount / batch.systemVerifiedCount) * 100, 100)
            : null;
                    ...
                    <div className="resend-detail-section">
                        ...
                        <div className="resend-count-cards">
                            <div className="resend-count-card">
                                <span className="resend-count-label">执行校验数量</span>
                                <span className="resend-count-value">{batch.userVerifiedCount.toLocaleString()}</span>
                            </div>
                            <div className="resend-count-card">
                                <span className="resend-count-label">系统校验数量</span>
                                <span className="resend-count-value">
                                    {batch.systemVerifiedCount === null ? '—' : batch.systemVerifiedCount.toLocaleString()}
                                </span>
                            </div>
                            <div className="resend-count-card">
                                <span className="resend-count-label">实际执行数量</span>
                                <span className="resend-count-value">
                                    {batch.executedCount === null ? '—' : batch.executedCount.toLocaleString()}
                                </span>
                            </div>
                        </div>
                        {progress !== null && (
                            <div className="resend-progress">
                                ...
                                <span className="resend-progress-text">
                                    已发送 {batch.executedCount?.toLocaleString()} / {batch.systemVerifiedCount?.toLocaleString()}
                                </span>
                            </div>
                        )}
                    </div>
